...
public class SdpToJingle {
    ...
    private static SessionDescription getNewSessionDescription(final String sid) throws SDPException, UnknownHostException {
         ...
         */
        Origin origin = new Origin("-", ntpTime, ntpTime, "0.0.0.0");
        try {
            origin.setSessionID(Long.parseLong(sid));
        } catch (NumberFormatException nfe) {
            /*
             * A random session identifier generated by the initiator, which effectively maps to the local-part of a SIP
             * "Call-ID" parameter; this SHOULD match the XML Nmtoken production [23] so that XML character escaping is
             * not needed for characters such as '&'. In some situations the Jingle session identifier might have security
             * implications. See RFC 4086 [24] regarding requirements for randomness.
             */
        }
    ...
    public static SessionDescription sdpFromJingle(JingleIQ jingle) {
        try {
            ...
            for (ContentPacketExtension content : contents) {
                ...
                // Note that webrtc data channels do not have description or payload-type elements
                // Chrome shows the equivalent SDP media as:
                // m=application 1 DTLS/SCTP 5000
                // c=IN IP4 0.0.0.0
                // a=ice-ufrag:1bj531ai9heb4h
                // a=ice-pwd:52t9uobp1rvtc5u2rj9ngjoqck
                // a=fingerprint:sha-1 F9:3F:AB:E6:B9:DB:C3:FD:62:AA:6F:50:92:7D:6E:CF:A0:BE:E9:73
                // a=setup:actpass
                // a=sctpmap:5000 webrtc-datachannel 1024
                // a=mid:data
                // a=candidate:1 1 SSLTCP 2130706431 172.18.10.26 4443 typ host generation 0
                // a=candidate:3 1 UDP 2130706431 172.18.10.26 10000 typ host generation 0
                // a=candidate:2 1 SSLTCP 1694498815 54.89.50.245 4443 typ srflx raddr 172.18.10.26 rport 4443 generation 0
                // a=candidate:4 1 UDP 1677724415 54.89.50.245 10000 typ srflx raddr 172.18.10.26 rport 10000 generation 0
                if (descriptionExts.isEmpty()) continue;
                ...
                if (payloadExts.size() > 0) {
                    String profile = descriptionExt.getProfile();
                    // Profile isn't a required attribute. See RFC-5124
                    if (profile == null) {
                        profile = "RTP/SAVPF";
                    }
                    media = new Media(contentType, 9, profile,
                ...
                if (!rawUdpExts.isEmpty()) {
                    ...
                    // Only ICE candidates
                    Connection connection = new Connection("0.0.0.0");
                    mediaDescription.setConnection(connection);
                    Attribute rawUdpAttr = new Attribute("rtcp", "9 IN IP4 0.0.0.0");
                    mediaDescription.addAttribute(rawUdpAttr);
                }
